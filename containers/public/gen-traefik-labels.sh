...
echo "    labels:
      - \"traefik.enable=true\"
      - \"traefik.http.routers.$SERVICE_NAME.rule=Host(\`$HOSTNAME.\$TRAEFIK_PUBLIC_DOMAIN\`)\"
      ...
      - \"traefik.http.routers."$SERVICE_NAME"_sec.rule=Host(\`$HOSTNAME.\$TRAEFIK_PUBLIC_DOMAIN\`)\"
